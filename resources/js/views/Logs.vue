<template>
    <div>
        ...
        <div v-if="logs.data.length > 0" class="grid gap-6 md:grid-cols-12 mb-4">
            <div class="relative h-9 md:col-span-4">
                ...
                <RoundInput
                    ...
                    :placeholder="__('Search')" spellcheck="false" :aria-label="__('Search')" type="search"
                    @input="search = $event.target.value" @keydown.stop="performSearch" />
            </div>
            ...
            <div v-if="!loading && files.length" class="md:col-span-4 md:justify-end flex items-center">
                <button v-if="permissions.canDownload" @click.prevent="download" :title="__('Download')"
                    class="cursor-pointer text-70 hover:text-primary mr-3">
                    <Icon type="download" view-box="0 0 24 24" width="24" height="24" />
                </button>
                <button v-if="permissions.canDelete" :title="__('Delete')"
                    class="cursor-pointer text-70 hover:text-primary mr-3" @click.prevent="openDeleteModal">
                    <Icon type="trash" view-box="0 0 24 24" width="20" height="20" />
                </button>

                <SelectControl size="lg" :options="files" v-model:selected="file" @input="file = $event.target.value"
                    @change="changeFile" />
            </div>
        </div>
        ...
        <div class="relative" :class="{ 'overflow-hidden': loading }">
            ...
            <template v-else>
                <Card>
                                ...
                                <tr class="hover:bg-blue-lightest">
                                    <td
                                        ...
                                        <span class="whitespace-no-wrap flex items-center">
                                            <Icon :type="icons[log.level || 'error']" view-box="0 0 24 24" width="24"
                                                height="24" :style="{ color: colors[log.level || 'error'] }" />
                    ...
                    <div v-if="!logs.data.length" class="flex flex-col justify-center items-center px-6 py-8">
                    ...
                    <div class="bg-20 rounded-b">
                        <nav v-if="logs.data.length > 0" class="flex justify-between">
                            <!-- Previous Link -->
                            <button :disabled="!hasPreviousPages" class="btn btn-link py-3 px-4" :class="{
                                    'text-primary dim': hasPreviousPages,
                                    'text-80 opacity-50': !hasPreviousPages
                                }" rel="prev" @click.prevent="selectPreviousPage()" dusk="previous">
                                {{ __('Previous') }}
                            </button>
                            ...
                            <!-- Next Link -->
                            <button :disabled="!hasMorePages" class="btn btn-link py-3 px-4" :class="{
                                    'text-primary dim': hasMorePages,
                                    'text-80 opacity-50': !hasMorePages
                                }" rel="next" @click.prevent="selectNextPage()" dusk="next">
                                {{ __('Next') }}
                            </button>
                        </nav>
                    </div>
        ...
        <Modal :show="showLog" tabindex="-1" data-testid="show-log" role="dialog" maxWidth="screen-md">
            <div class="mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
                ...
                <ModalFooter class="flex items-center justify-center">
                    <CancelButton component="button" type="button" dusk="cancel-action-button"
                        @click.prevent="showLog = null" />
                </ModalFooter>
            </div>
        </Modal>

        <DeleteResourceModal mode="delete" :show="deleteModalOpen" @close="closeDeleteModal" @confirm="confirmDelete">
