export default {
    getLogs(file = null, current_page = 1, search = null) {
        return Nova.request()
            .get(
                search
                    ? `/nova-vendor/stepanenko3/logs-tool/logs?file=${file}&page=${current_page}&search=${search}`
                    : `/nova-vendor/stepanenko3/logs-tool/logs?file=${file}&page=${current_page}`
            )
            .then(response => response.data);
    },
    getDailyLogFiles(file = null, current_page = 1) {
        return Nova.request()
            .get(`/nova-vendor/stepanenko3/logs-tool/daily-log-files`)
            .then(response => response.data);
    },
    deleteFile(file) {
        return Nova.request()
